...
// Всі таймаути, rate-limit, duration — виносяться сюди.
// Адмін може змінювати settings.json без перезапуску (читається при кожному запиті).
const DEFAULT_SETTINGS = {
  // Rate limits
  photoRateLimitMs:        15000,   // мінімум між фото-генераціями
  videoRateLimitMs:        60000,   // мінімум між відео-генераціями
  aiPromptRateLimitMs:     30000,   // мінімум між AI-промт запитами
  ...
  // Денні ліміти відео (0 = без ліміту)
  ...
  videoEnabled:            true,
  // Денний ліміт AI-промтів (0 = без ліміту)
...
function appendLog(entry) {
  try {
    fs.appendFileSync(GEN_LOG_PATH, JSON.stringify(entry) + "\n", "utf8");
...
// ─── ПАКЕТИ ───────────────────────────────────────────────────────────────────
// count замість photos — семантично правильно для відео-пакетів
const PACKAGES = {
...
// ─── RATE LIMITING (окремо для фото / відео / AI-промт) ───────────────────────
const userLastPhoto    = {};
const userLastVideo    = {};
const userLastAiPrompt = {};
const userGenerating   = new Set();
// Spam photo protection: лічимо кількість фото за вікно часу
const userPhotoSpam    = {}; // { userId: [timestamp, ...] }
const SPAM_WINDOW_MS   = 10000; // 10 сек
const SPAM_MAX_PHOTOS  = 3;     // макс фото за вікно

function checkPhotoSpam(userId) {
  const now  = Date.now();
  const times = (userPhotoSpam[userId] || []).filter(t => now - t < SPAM_WINDOW_MS);
  userPhotoSpam[userId] = times;
  if (times.length >= SPAM_MAX_PHOTOS) return true; // спам
...
function checkRateLimit(userId, type) {
  ...
  const now = Date.now();
  if (map[userId] && now - map[userId] < ms) {
    return Math.ceil((ms - (now - map[userId])) / 1000);
  }
  return 0;
}
...
function checkDailyVideoLimit(user, model) {
  ...
  if (limit === 0) return null; // без ліміту

  // Скидаємо лічильник якщо новий день
  if (user.dailyVideoDate !== today) {
    user.dailyVideoDate     = today;
    user.dailySeedanceCount = 0;
    user.dailyKlingCount    = 0;
  }

  const used = model === "kling" ? (user.dailyKlingCount || 0) : (user.dailySeedanceCount || 0);
  if (used >= limit) {
    return `❌ Денний ліміт ${model === "kling" ? "Kling" : "Seedance"}: ${limit} відео/день.\nСпробуй завтра або купи більший пакет.`;
  }
  return null;
}
...
function checkDailyAiLimit(user) {
  ...
  if ((user.dailyAiCount || 0) >= limit) {
    return `❌ Денний ліміт AI-промтів: ${limit}/день. Спробуй завтра або напиши промт вручну через ✍️ Свій промт.`;
  }
  return null;
}
...
function incrementDailyVideo(user, model) {
  const today = getTodayDate();
  if (user.dailyVideoDate !== today) {
    user.dailyVideoDate = today; user.dailySeedanceCount = 0; user.dailyKlingCount = 0;
  }
...
// totalVideoGenerations — сума seedance + kling
function getUserVideoTotal(user) {
  return (user.seedanceGenerations || 0) + (user.klingGenerations || 0);
}
...
async function handleReferral(ctx, referrerId) {
  ...
  if (newUser.referredBy) return; // вже є реферер

  // ── АНТИ-АБ'ЮЗ ────────────────────────────────────────────────────────────
  // 1. Реферер не може мати > 20 рефералів до першої оплати (bot-farming)
  if (!referrer.totalSpent && (referrer.referralCount || 0) >= 20) {
    console.warn(`REFERRAL ABUSE: user ${referrerId} hit limit without payment`);
    return;
  }
  // 2. Один IP може дати не більше 5 бонусів (Telegram не дає IP,
  //    тому перевіряємо username: якщо у рефера немає username — підозрілий)
  //    Це м'яка евристика, не блокує, але не нараховує бонус.
  const referrerHasUsername = !!referrer.username;
  const newUserHasUsername  = !!(ctx.from.username);
  const grantBonus = referrerHasUsername; // бонус тільки якщо реферер реальний
  ...
  if (grantBonus) {
    ...
    try {
      await bot.telegram.sendMessage(referrerId,
        `🎉 Новий друг зареєструвався!\n+${REFERRAL_PHOTO_BONUS} фото 🖼\nБаланс фото: ${referrer.balance}`);
    } catch (e) { console.error("REFERRAL NOTIFY:", e.message); }
  } else {
    saveUsers();
    console.log(`REFERRAL: user ${newUserId} referred by ${referrerId} — bonus skipped (no username)`);
  }
}
...
// Retry для Telegram send (replyWithPhoto / replyWithVideo)
...
// Гнучкий парсер: підтримує будь-який префікс пакету
...
function signWfpCallback(data) {
  const vals = [data.merchantAccount||"", data.orderReference||"", data.amount||"", data.currency||"", data.authCode||"", data.cardPan||"", data.transactionStatus||"", data.reasonCode||""];
  return crypto.createHmac("md5", WAYFORPAY.secretKey).update(vals.join(";"), "utf8").digest("hex");
}
...
function normalizeWayForPayCallbackBody(body) {
  ...
  for (const key of Object.keys(body)) {
    if (key.includes("merchantAccount"))                              { try { return JSON.parse(key);       } catch {} }
    if (typeof body[key] === "string" && body[key].includes("merchantAccount")) { try { return JSON.parse(body[key]); } catch {} }
    if (key.startsWith("{"))                                          { try { return JSON.parse(key);       } catch {} }
  }
  return body;
}
...
const adminMenu       = () => Markup.keyboard([["📊 Статус бота", "👤 Мій ID"], ["👥 Користувачі", "💳 Останні оплати"], ["✏️ Змінити промт", "📝 Поточні промти"], ["✏️ Змінити текст", "📝 Поточні тексти"], ["⚙️ Налаштування", "📊 Баланс"], ["↩️ Назад"]]).resize();
...
bot.command("broadcast", async (ctx) => {
  ...
  for (const u of all) {
    try { await bot.telegram.sendMessage(u.id, text, mainMenu()); sent++; await new Promise(r => setTimeout(r, 50)); }
    catch { failed++; }
  }
...
bot.hears("👤 Мій ID", (ctx) => {
  touchUser(ctx);
  if (!isAdmin(ctx.from.id)) return ctx.reply("❌");
  return ctx.reply(`Твій ID: ${ctx.from.id}`, adminMenu());
});
...
// ⚙️ Налаштування — показати settings.json
...
bot.action("upsell_seedance", async (ctx) => {
  try {
    ...
    ensureSession(ctx); ctx.session.mode = "video"; ctx.session.style = "seedance";
    await ctx.reply("Обери пакет Seedance 🎬:", buySeedanceMenu());
...
bot.action("upsell_kling", async (ctx) => {
  try {
    await ctx.answerCbQuery();
    ensureSession(ctx); ctx.session.mode = "video"; ctx.session.style = "kling";
    await ctx.reply("Обери пакет Kling 🎥:", buyKlingMenu());
...
const ALL_BUTTONS = [
  ...
  "✏️ Змінити промт","📝 Поточні промти","✏️ Змінити текст","📝 Поточні тексти","⚙️ Налаштування",
...
bot.on("photo", async (ctx) => {
  ...
  if (ctx.session.awaitingAiPrompt) {
    ...
    try {
      ...
      if (!suggested) {
        return ctx.reply("❌ Не вдалося згенерувати промт. Спробуй ✍️ Свій промт.", aiMode === "video" ? videoMenu() : photoMenu());
      }
  ...
  // Ліміт черги — захист від перевантаження
  const MAX_QUEUE = 50;
  const queuePos = generationQueue.length;
  if (queuePos >= MAX_QUEUE) {
    return ctx.reply("⚠️ Сервіс зараз перевантажений. Спробуй через кілька хвилин.");
  }

  if (queuePos > 0) await ctx.reply(`⏳ Черга: #${queuePos + 1}. Зачекай...`);

  userGenerating.add(userId);
  try {
    await enqueueGeneration(userId, () => _processGeneration(ctx, user, userId, mode, photo));
  } catch (e) { userGenerating.delete(userId); }
});
  ...
  try {
    ...
    if (mode === "video") {
      ...
      try {
        ...
        // Апсел з inline кнопками якщо баланс низький
    ...
    // М'який апсел на відео
    await tgSendWithRetry(() => ctx.replyWithPhoto({ url }, { caption }));

    // М'який апсел на відео кожні 3 генерації
...
app.post("/payment", async (req, res) => {
  try {
    ...
    if (txStatus === "approved" && !isCredited(data.orderReference)) {
      markAsCredited(data.orderReference); // атомарно першим
      ...
      user.pendingOrderReference = null; user.lastPaidAt = new Date().toISOString();
      ...
      try { await bot.telegram.sendMessage(userId, `✅ Оплату підтверджено!\n\n${emoji} ${pack.title}\nЗараховано: ${pack.count}\n${balanceLabel}: ${balanceNow}`, mainMenu()); }
      catch (e) { console.error("SEND USER MSG:", e.message); }

      for (const adminId of ADMINS) {
        try { await bot.telegram.sendMessage(adminId, `✅ Оплата!\nUser: ${userId} @${user.username || "-"}\n${pack.title} | ${data.amount} грн\n${data.orderReference}`); }
...
const BACKUP_FILES = [USERS_PATH, PAYMENTS_PATH, PAYMENT_LOCK_PATH];
...
setInterval(runBackup, 6 * 60 * 60 * 1000); // кожні 6 годин

bot.launch({ dropPendingUpdates: true })
  .then(() => {
    console.log("🔥 Бот запущений (v4)");
    saveJson(SETTINGS_PATH, loadSettings());
    runBackup(); // backup при старті
  })
